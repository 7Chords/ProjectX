public class SCMsgConst
{

    #region 回合制战斗相关 20
    public const int TBS_GAME_START = 2000;
    public const int TBS_GAME_FINISH = 2001;

    public const int TBS_TURN_CHG = 2002;//回合制战斗 - 回合轮转
    public const int TBS_ACTOR_CHG = 2003;//回合制战斗 - 角色轮转

    public const int TBS_TURN_WORK = 2004;
    public const int TBS_ACTOR_WORK = 2005;
    public const int TBS_EFFECT_WORK = 2006;
    public const int TBS_COMP_WORK = 2007;

    public const int TBS_TURN_REST = 2008;
    public const int TBS_ACTOR_REST = 2009;
    public const int TBS_EFFECT_REST = 2010;
    public const int TBS_COMP_REST = 2011;

    public const int TBS_GAME_RESULT = 2012;

    #endregion

    #region 系统相关 99
    public const int GAME_START = 9901;
    public const int GAME_END = 9902;


    #endregion
}
